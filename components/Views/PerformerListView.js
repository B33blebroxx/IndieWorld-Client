...
function PerformerList({
  performers, handleRemove, showPromotionId, userPromotionId,
}) {
  return (
    <div className="performer-card-container">
      {performers.map((performer) => (
        <PerformerCard key={performer.id} performer={performer} onRemove={handleRemove} showPromotionId={showPromotionId} userPromotionId={userPromotionId} />
      ))}
    </div>
  );
}
